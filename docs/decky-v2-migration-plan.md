...
- 状态：阶段 4.1 全量状态同步真机验收通过，准备阶段 4.2 非关键事件推送试点
- 目标分支：`v2`
- 编写日期：2026-07-11
- 最近更新：2026-07-11
...
- 先将 `SettingsChanged` 迁移至 `decky.emit()` 和前端事件监听器。
- 前端卸载时必须移除监听器；丢失该事件后可由阶段 4.1 的全量同步恢复。
- 真机验证面板关闭、重新打开以及 Loader/Steam UI 重启路径。

#### 阶段 4.3：关键状态变化信号

- 抑制状态事件只作为“状态已变化”信号，前端收到后重新获取完整状态，不依赖可能丢失或乱序的边沿事件载荷。
- 保留串行电源操作队列和失败后的状态重取机制。
- 在推送与长轮询并存期间避免重复通知和重复电源写入。
...
V2 应采用完整 ESM 迁移，不在旧 IIFE 架构上继续包装 legacy API。阶段 0 的最小兼容性验证、阶段 1 的现代 UI 外壳迁移、阶段 2 的分组 RPC 迁移、阶段 3 的后端模块迁移、阶段 4.0 的发布基线整理以及阶段 4.1 的全量状态同步均已通过 Decky Loader 3.2.6 真机验收；下一步实施阶段 4.2，先以可由全量同步恢复的 `SettingsChanged` 作为非关键事件推送试点。
