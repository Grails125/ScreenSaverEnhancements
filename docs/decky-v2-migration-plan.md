...
- 状态：阶段 4.4 长轮询移除及推送迁移真机验收通过，V2 迁移完成
- 目标分支：`v2`
- 编写日期：2026-07-11
- 最近更新：2026-07-11
...
- `InhibitStateChanged` 已迁移为 `inhibit_state_changed` Decky 推送；前端卸载时精确移除设置和抑制状态两类监听器。
- 已删除 `wait_for_events`、`PluginEvent`、后端共享事件队列、事件信号以及长轮询请求/超时/队列诊断字段。
- 原长轮询承担的失联 D-Bus 客户端清理由独立的 25 秒连接巡检接管，失联请求清除后发出完整状态重取信号。
- 诊断页改为显示推送监听状态、监听器重连次数和最近一次全量同步的时间与结果；首次订阅不计为重连，事件处理失败后两条推送监听器重新订阅成功时计数加一并执行全量状态同步，诊断导出同步采用新字段。
- 阶段前回归审计发现的问题已处理：关闭监控会等待 DeckyMusic 跟踪和电源状态收敛后再通知；误导性的 `uptimeSeconds` 字段与 `shouldStopInhibit` 死代码已完全删除。
- Decky Loader 3.2.6 真机验证中，稳定 D-Bus 请求通过推送正确建立并清除电源恢复快照；短命客户端由独立巡检清除，未残留抑制状态。
- 验证结束后设置哈希恢复为部署前的 `2604b9...b643`，系统电源超时保持为 300/300/3600/3600，Loader 中两类推送监听器各且仅有一个。
- 完整插件与设置备份位于 `/home/deck/homebrew/backups/screensaver-stage4-4-push-20260711-fca4ecc`。
...
V2 完整 ESM 迁移已经完成。阶段 0 的兼容性验证、阶段 1 的现代 UI 外壳、阶段 2 的分组 RPC、阶段 3 的现代后端模块，以及阶段 4.0 至 4.4 的发布基线、全量同步、事件推送与长轮询移除均已通过 Decky Loader 3.2.6 真机验收。当前实现不再依赖 `decky-frontend-lib`、legacy RPC 适配器或插件事件长轮询。
