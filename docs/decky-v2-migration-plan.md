...
V2 应采用完整 ESM 迁移，不在旧 IIFE 架构上继续包装 legacy API。阶段 0 的最小兼容性验证、阶段 1 的现代 UI 外壳迁移以及阶段 2 的分组 RPC 迁移均已通过 Decky Loader 3.2.6 真机验收；下一步迁移后端 Decky 模块，最后独立评估事件推送。
