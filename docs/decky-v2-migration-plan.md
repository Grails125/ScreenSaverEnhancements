...
V2 应采用完整 ESM 迁移，不在旧 IIFE 架构上继续包装 legacy API。阶段 0 的最小兼容性验证、阶段 1 的现代 UI 外壳迁移、阶段 2 的分组 RPC 迁移以及阶段 3 的后端模块迁移均已通过 Decky Loader 3.2.6 真机验收；下一步独立评估可选的事件推送迁移。
