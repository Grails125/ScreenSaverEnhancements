...
- 状态：阶段 4.2 非关键事件推送真机验收通过，准备阶段 4.3 关键状态变化信号
- 目标分支：`v2`
- 编写日期：2026-07-11
- 最近更新：2026-07-11
...
V2 应采用完整 ESM 迁移，不在旧 IIFE 架构上继续包装 legacy API。阶段 0 的最小兼容性验证、阶段 1 的现代 UI 外壳迁移、阶段 2 的分组 RPC 迁移、阶段 3 的后端模块迁移、阶段 4.0 的发布基线整理、阶段 4.1 的全量状态同步以及阶段 4.2 的非关键事件推送均已通过 Decky Loader 3.2.6 真机验收；下一步实施阶段 4.3，将关键状态事件收敛为触发完整状态重取的变化信号。
